defmodule MistWeb.RelayLive.Index do
  ...
  alias Mist.Nostr
  alias Mist.Nostr.{Relay, RelayInfo, RelayStatus}
  ...
  defp apply_action(socket, :index, _params) do
    relays = RelayManager.get_states()
      |> Enum.with_index(fn x, i ->
          case RelayInfo.get(x.url) do
            {:ok, info} -> Map.merge(%RelayStatus{id: i, relay_info: info}, x) |> dbg()
            {:error, _reason} -> Map.merge(%RelayStatus{id: i, relay_info: "not available"}, x)
          end
        end)
